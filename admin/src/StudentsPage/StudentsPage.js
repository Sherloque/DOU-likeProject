        ...
        return (
            <><p>STUDENTS</p>
                {(students) ? students.map((item, i) => (<div key={i}><p></p>
                    ...
                    {(item.recommendation) ? (null) : (<><p>Вы не внесли ему рекомендацию</p>
                    </>)}
                    <button className=''><Link to={{pathname:`/profile/${item._id}`, state: {student:item}}}  style={{ color: "white", textDecoration: 'none' }}>Профиль студента</Link></button>
                    </div>))
